...
class OpenAIGateway:
    ...
    async def rewrite_offer(self, original_text: str) -> str:
        prompt = f"""
            ...
            Текст не должен быть слишком скучный. Если итоговый кэшбек для покупателя > 50%, то выдели его жирным и поставь 🔥
            Разделяй пункты: цена, кэшбек, ссылку на аккаунт для связи.
            Если предложение очень выгодно для покупателя(большой кэшбек), то поставь больше эмодзи.  
